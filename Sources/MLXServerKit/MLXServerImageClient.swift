...
public enum MLXServerImageError: Error, LocalizedError, CustomStringConvertible {
...
public final class MLXServerImageClient {
    ...
    private func post<Payload: Encodable>(
        ...
        var fallbackError: MLXServerImageError?

        for path in paths {
            do {
                return try await post(payload, path: path)
            } catch let error as MLXServerImageError where error.statusCode == 404 || error.statusCode == 405 {
                fallbackError = error
                continue
            }
        }

        throw fallbackError ?? MLXServerImageError.invalidResponse
    }
    ...
    private func post<Payload: Encodable>(
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw MLXServerImageError.invalidResponse
        }
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw MLXServerImageError.httpStatus(httpResponse.statusCode, String(decoding: data, as: UTF8.self))
        }
