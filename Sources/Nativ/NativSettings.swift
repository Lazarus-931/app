...
struct ServerAPIKeychain: ServerAPICredentialStoring {
    ...
    func load() throws -> String? {
        ...
        return Self.normalized(token)
    }

    func save(_ token: String?) throws {
        guard let token = Self.normalized(token) else {
...
struct NativSettings: Codable, Equatable {
    ...
    static func load(
        ...
        let legacyToken = Self.normalizedModelID(storedSettings.serverAPIKey)
