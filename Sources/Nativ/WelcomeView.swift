...
private struct WelcomeView: View {
    ...
    private var recommendedModels: [HuggingFaceModel] {
        ...
        return Array(candidates.sorted { lhs, rhs in
            let lhsFits = lhs.sizeBytes.map { ModelCapacity.tier(weightBytes: $0) != .tooBig } ?? true
            let rhsFits = rhs.sizeBytes.map { ModelCapacity.tier(weightBytes: $0) != .tooBig } ?? true
            if lhsFits != rhsFits {
                return lhsFits
            }
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 7) {
                        ...
                        if let sizeBytes = model.sizeBytes,
                           ModelCapacity.tier(weightBytes: sizeBytes) == .tooBig {
                            Text("Too big")
